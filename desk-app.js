...
async function simulate(){
 ...
 const t=(document.getElementById('ticker').value||'NVDA').trim().toUpperCase();
 if(!walletAddress){el.textContent='Connect the execution wallet before chain simulation.';return}
 if(!preflightReady){el.textContent='Run preflight first. Simulation stays locked until guardrails pass.';return}
 el.textContent='Running dry-run simulation…';
 try{
  ...
  el.innerHTML='<b>SIMULATION PASSED</b>\n'+JSON.stringify({mode:j.mode,simulationMode:j.simulationMode,broadcast:j.broadcast,next:j.next,plan:j.plan},null,2);
  const s=document.getElementById('poaGateStatus');
  if(s)s.textContent='Simulation passed. Confirmation still requires explicit user approval.';
  const btn=document.getElementById('confirmActionBtn');
  if(btn)btn.disabled=false;
 }catch(e){el.textContent='Simulation error: '+e.message}
}
async function confirmAction(){
 const s=document.getElementById('poaGateStatus');
 if(!walletAddress){if(s)s.textContent='Connect the execution wallet before confirmation.';return}
 if(!preflightReady){if(s)s.textContent='Preflight must pass before confirmation.';return}
 if(!window.__pronousSimulated){if(s)s.textContent='Run simulation before confirmation. Server never broadcasts.';return}
 setExecutionStep('confirmation','CONFIRMED');
 if(s)s.textContent='User confirmed intent. Creating POA evidence (no broadcast).';
 createCurrentPOA('CONFIRMED');
}
async function loadOnchain(x){
 ...
 box.textContent='Loading on-chain context…';
 try{
  const r=await fetch('/api/onchain?token='+encodeURIComponent(token));
  const j=await r.json().catch(()=>({}));
  box.innerHTML='<div><b>Contract</b> '+esc(token)+'</div><div class="muted small" style="margin-top:6px">Network BSC · '+esc(x.platformId||'')+' · '+esc(x.tokenSymbol||'')+'</div>'+(j.error?'<div class="neg" style="margin-top:8px">'+esc(j.error)+'</div>':'<div class="pos" style="margin-top:8px">Contract resolved for inspection.</div>');
...
async function createCurrentPOA(status){
 const box=document.getElementById('poaCurrent');
 const t=(document.getElementById('ticker').value||'NVDA').trim().toUpperCase();
 const intent={ticker:t,amount:document.getElementById('amount')&&document.getElementById('amount').value,maxSpend:document.getElementById('maxSpend')&&document.getElementById('maxSpend').value,wallet:walletAddress||null,preflightReady:!!preflightReady,simulated:!!window.__pronousSimulated};
 const input={action:'SCAN_OR_TRADE',status:status||(window.__pronousSimulated?'SIMULATED':'PLANNED'),network:'BSC',asset:{ticker:t},intent,userConfirmation:status==='CONFIRMED'};
 if(box)box.textContent='Creating POA…';
 try{
  const r=await fetch('/api/poa?action=create&proof='+encodeURIComponent(JSON.stringify(input)));
  const j=await r.json();
  if(!r.ok||j.error)throw new Error(j.error||'POA create failed');
  currentPOA=j.proof;
  if(box)box.innerHTML='<div class="poa-proof"><div><b>'+esc(j.proof.poaId)+'</b> · '+esc(j.proof.status)+'</div><code>'+esc(j.proof.poaHash)+'</code><button class="secondary" type="button" onclick="verifyCurrentPOA()">Verify</button><div id="poaVerify" class="poa-verify"></div></div>';
  const ledger=loadPOALedger();
  ledger.unshift({poaId:j.proof.poaId,status:j.proof.status,hash:j.proof.poaHash,at:j.proof.timestamp});
  savePOALedger(ledger);
  renderPOALedger();
 }catch(e){if(box)box.textContent='POA error: '+e.message}
}
async function verifyCurrentPOA(){
 const el=document.getElementById('poaVerify');
 if(!currentPOA){if(el)el.textContent='No POA in session.';return}
 try{
  const r=await fetch('/api/poa?action=verify&proof='+encodeURIComponent(JSON.stringify(currentPOA)));
  const j=await r.json();
  const v=j.verification||{};
  if(el){el.textContent=v.valid?'VALID · hash match':'INVALID · '+(v.reason||'mismatch');el.className='poa-verify '+(v.valid?'valid':'invalid')}
 }catch(e){if(el)el.textContent='Verify error: '+e.message}
}
...
function renderPOALedger(){
 const el=document.getElementById('poaLedger');if(!el)return;
 const rows=loadPOALedger();
 if(!rows.length){el.textContent='No evidence yet.';return}
 el.innerHTML=rows.map(r=>'<div class="poa-row"><div><b>'+esc(r.poaId)+'</b> <span class="muted">'+esc(r.status)+'</span></div><code>'+esc(String(r.hash||'').slice(0,16))+'…</code><span class="muted small">'+esc(String(r.at||'').slice(0,19))+'</span></div>').join('');
}
loadMarket();
loadSkills();
initMarketWatch();
