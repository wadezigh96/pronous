...
function PrivyBridge(){
  ...
  useEffect(()=>{
    ...
    privyDisconnect=async()=>{
      try{if(authenticated)await logout()}
      finally{
    ...
    (async()=>{
      try{
        ...
        if(String(chain).toLowerCase()!=='0x38'){
          ...
          catch(e){throw new Error('PRIVY_WALLET_MUST_USE_BSC_56')}
...
function mount(){
  ...
  try{
    ...
    createRoot(el).render(React.createElement(
      PrivyProvider,
      {appId:PRIVY_APP_ID,config},
      React.createElement(PrivyBridge)
    ));
  }catch(e){
    privyError=e?.message||String(e);
    ui('PRIVY ERROR: '+privyError,true);
...
window.connectWallet=async function(){
  ...
  try{
    ...
    ui(privyReady?'OPENING PRIVY':'PRIVY LOADING');
    ...
    if(btn&&!window.__pronousPrivyConnected)btn.disabled=false;
  }
};
