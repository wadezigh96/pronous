...
function createPOA(input = {}) {
  ...
  if (status === "EXECUTED" && !proof.txHash) throw new Error("EXECUTED_POA_REQUIRES_TX_HASH");
...
function verifyPOA(proof = {}) {
  ...
  const actual = Buffer.from(String(proof.poaHash), "utf8");
  const expectedBuffer = Buffer.from(expected, "utf8");
  const lengthMatch = actual.length === expectedBuffer.length;
  const valid = lengthMatch && crypto.timingSafeEqual(expectedBuffer, actual);
  return {
    valid,
