...
function parseEd25519PrivateKey(secret) {
  // Vercel env vars may contain literal escaped newlines (\\n).
  // Normalize them before handing the key to Node/OpenSSL.
  ...
  // Accept the PKCS#8 PEM form generated by Binance tooling.
  ...
  // Also accept a base64-encoded PKCS#8 DER private key.
  ...
  if (/^[A-Za-z0-9+/=]+$/.test(compact)) {
    ...
    try {
      ...
      // Some integrations store only the 32-byte Ed25519 seed. Wrap it
      // in the standard PKCS#8 DER container before passing it to OpenSSL.
      if (der.length === 32) {
